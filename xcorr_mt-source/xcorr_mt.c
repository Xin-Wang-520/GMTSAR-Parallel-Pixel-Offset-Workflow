 ...
 * processes and merges their output in order, producing byte-
 * identical freq_xcorr.dat / time_xcorr.dat compared to the
 * original single-process xcorr.
 *
...
int main(int argc, char **argv) {
	...
	int cli_nproc, nproc, k, spawned, failed;
	struct xcorr *xc;
	...
	/* pull -nproc out of argv before the original argument parsing */
	...
	/* cap worker count: CPU-bound workers gain nothing beyond the online
	 * core count, while their memory footprint scales with nproc        */
	{
		long cores = sysconf(_SC_NPROCESSORS_ONLN);
		if (cores < 1)
			cores = 1;
		if (nproc > cores) {
			fprintf(stderr, " nproc %d exceeds %ld online cores; clamped\n", nproc, cores);
			nproc = (int)cores;
		}
	}
	if (nproc > xc->nlocs)
	...
	/* single process: identical behavior to the original xcorr */
	if (nproc == 1) {
	...
	/* multi-process path: fork workers over contiguous iloc chunks */
	...
	for (k = 0; k < nproc; k++) {
		int il0 = (int)((long)k * xc->nlocs / nproc);
		int il1 = (int)((long)(k + 1) * xc->nlocs / nproc);
		pid_t pid = fork();
		...
		if (pid == 0) {
			/* child: private copy of everything; reopen inputs to get a  */
			/* private file offset (fork shares the open file description) */
			char partname[512];
			...
			snprintf(partname, sizeof(partname), "%s.part.%d", xc->filename, k);
			xc->file = fopen(partname, "w");
			if (xc->file == NULL)
				die("Can't open output file", partname);
			...
			/* _exit: do not flush inherited stdio buffers nor run atexit */
			_exit(EXIT_SUCCESS);
		}
	...
	/* parent: collect all workers */
	failed = (spawned < nproc) ? 1 : 0;
	for (k = 0; k < spawned; k++) {
		int status;
		if (waitpid(pids[k], &status, 0) < 0 || !WIFEXITED(status) || WEXITSTATUS(status) != 0)
			failed = 1;
	}
	...
	if (failed) {
		...
		die(" a worker process failed; no output written\n", "");
	}

	/* merge the per-worker parts in iloc order: byte-identical output */
	{
		FILE *out;
		char buf[65536];

		out = fopen(xc->filename, "w");
		if (out == NULL)
			die("Can't open output file", xc->filename);
		...
		for (k = 0; k < nproc; k++) {
			...
			snprintf(partname, sizeof(partname), "%s.part.%d", xc->filename, k);
			if ((pf = fopen(partname, "r")) == NULL) {
				fclose(out);
				die("Can't open part file", partname);
			}
